...
Texture<Real> parse_float_texture(
    ...
    } else if (type == "rgb" || type == "srgb" || type == "spectrum") {
        return make_constant_float_texture(avg(parse_color(node, default_map)));
    } else if (type == "texture") {
        ParsedTexture t = parse_texture(node, default_map);
        // increment ref_id_counter until we can't find the name in texture_pool
        static int ref_id_counter = 0; // should switch to atomic<int> if we parse scenes using multiple threads
...
std::tuple<std::string /* ID */, Medium> parse_medium(
    ...
    if (type == "homogeneous") {
    ...
    } else if (type == "glowdischarge") {
        // #Edited (Phase 1 & Phase 2)
        // Description: Parses glow discharge medium parameters from XML scene file
        // Glow discharge parameters
        Real alpha = 100.0;   // Ionization coefficient (Phase 1)
        Real beta = 1e-8;     // Recombination coefficient (Phase 1)
        Real eta = 0.5;       // Attachment coefficient (Phase 2) - electron → negative ion
        Real rho = 100.0;     // Ion scale coefficient (Phase 2) - ions move rho times slower
        Real sigma_0 = 4.2e-19;
        Vector3 drift_center{0, 0, 0};
        Real drift_magnitude = 1.0;
        Vector3 drift_direction{0, 1, 0};
        Real boundary_radius = 1.0;
        Vector3 emission_color{0.8, 0.3, 0.8}; // Pink/purple
        ...
        for (auto child : node.children()) {
            ...
            if (name == "alpha") {
                ...
                beta = parse_float(child.attribute("value").value(), default_map);
            } else if (name == "eta") {  // #Edited (Phase 2) - Parse attachment coefficient
                eta = parse_float(child.attribute("value").value(), default_map);
            } else if (name == "rho") {  // #Edited (Phase 2) - Parse ion scale coefficient
                rho = parse_float(child.attribute("value").value(), default_map);
            } else if (name == "sigma0") {
                sigma_0 = parse_float(child.attribute("value").value(), default_map);
            } else if (name == "driftCenter") {
                drift_center = parse_vector3(child.attribute("value").value(), default_map);
            } else if (name == "driftMagnitude") {
                drift_magnitude = parse_float(child.attribute("value").value(), default_map);
            } else if (name == "driftDirection") {
                drift_direction = parse_vector3(child.attribute("value").value(), default_map);
            } else if (name == "boundaryRadius") {
                boundary_radius = parse_float(child.attribute("value").value(), default_map);
            } else if (name == "emissionColor") {
                emission_color = parse_color(child, default_map);
            } else if (name == "integrationSteps") {
                integration_steps = (int)parse_float(child.attribute("value").value(), default_map);
            } else if (std::string(child.name()) == "phase") {
                phase_func = parse_phase_function(child, default_map);
            }
        }
...
Shape parse_shape(pugi::xml_node node,
                  ...
                  const std::map<std::string /* name id */, Spectrum> &emitter_params_map,
                  const std::map<std::string, std::string> &default_map) {
    int material_id = -1;
    ...
    for (auto child : node.children()) {
        ...
        if (name == "ref") {
            ...
            } else if (name_value == "emitter") {
                // Emitter references are handled later in the code (after shape creation)
                // Skip them here
            } else {
                // Assume this is a BSDF/material reference
    ...
    // Check for emitter reference or inline definition
    Spectrum radiance = fromRGB(Vector3{0, 0, 0}); // Default: no emission
    bool has_emitter = false;

    for (auto child : node.children()) {
        std::string name = child.name();
        if (name == "ref") {
            // Check if this is an emitter reference
            std::string ref_name = child.attribute("name").value();
            if (ref_name == "emitter") {
                pugi::xml_attribute id = child.attribute("id");
                if (id.empty()) {
                    Error("Emitter reference ID not specified.");
                }
                // Find the emitter in the emitter_params_map (will be added later)
                auto it = emitter_params_map.find(id.value());
                if (it == emitter_params_map.end()) {
                    Error(std::string("Emitter reference ") + id.value() + std::string(" not found."));
                }
                radiance = it->second;
                has_emitter = true;
            }
        } else if (name == "emitter") {
            // Inline emitter definition (original behavior)
            radiance = fromRGB(Vector3{1, 1, 1});
            for (auto grand_child : child.children()) {
                std::string name = grand_child.attribute("name").value();
                if (name == "radiance") {
...
std::unique_ptr<Scene> parse_scene(pugi::xml_node node, const RTCDevice &embree_device) {
    ...
    std::vector<Medium> media;
    std::map<std::string /* name id */, int /* index id */> medium_map;
    std::map<std::string /* name id */, Spectrum> emitter_params_map;
    ...
    for (auto child : node.children()) {
        ...
        if (name == "default") {
            ...
            if (type == "area") {
                // Scene-level area emitter definition with ID for referencing
                std::string id = child.attribute("id").value();
                if (!id.empty()) {
                    ...
                    emitter_params_map[id] = radiance;
                }
                // If no ID, this is an inline emitter (will be handled in shape parsing)
            } else if (type == "envmap") {
                std::string filename;
                Real scale = 1;
                Matrix4x4 to_world = Matrix4x4::identity();
